...
`moon-escape/` 把你在云端的工作空间流式拉到本地，落成符合 OKF 的 Markdown。
它本身又由两部分组成：
...
| Lark (飞书)  | [`bye-bye-lark/`](bye-bye-lark/)     | 🚧 仅初始化 |
| 语雀 (Yuque) | [`bye-bye-yuque/`](bye-bye-yuque/)   | 🚧 仅初始化 |
...
[`moon-app/`](moon-app/) 是跑在你浏览器里的本地 OKF Web 编辑器。
...
[`moon-shot/`](moon-shot/) 是把本地 OKF 知识重新发射出去的引擎，让 Agent / RAG / 检索真正用得上。
...
`moon-shot/okf/docs/` 里同时放着 **Google OKF 规范**（`OKF.md` 原文 + `OKF_CN.md` 中文翻译）—— OKF 是贯穿三个模块的统一契约，ESCAPE 写它、MOON 编辑它、SHOT 消费它。
...
| `pnpm web`       | 启 Web 编辑器（`moon-app` 的 `next dev`，即 MOON）         |
| `pnpm web:build` | 构建 Web 编辑器生产包                                  |

子包命令也可直接进 `moon-app/` 跑 `pnpm dev` / `pnpm test`，进 `moon-shot/` 跑它自己的命令，pnpm workspaces 自动提升依赖。
...
- **OKF 规范** —— [`moon-shot/okf/docs/OKF_CN.md`](moon-shot/okf/docs/OKF_CN.md)（中文翻译，源自 Google Cloud 博客 2026-06）。ESCAPE / MOON / SHOT 三个模块都遵循这份契约。
- **Notion 字段映射** —— [`bye-bye-notion/docs/`](bye-bye-notion/docs/)（PAGE / DATABASE / PAGE_PROPERTIES / NOTION_PAT）
- **SHOT 模块说明** —— [`moon-shot/README.md`](moon-shot/README.md)
